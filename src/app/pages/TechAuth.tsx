...
function StepAuth({
  ...
  if (loggedInUser) {
    return (
      <div>
        <div className="flex items-center gap-3 mb-5 p-4 bg-[#f0f7f4] border border-[#c8dfd8] rounded-2xl">
          <CheckCircle size={24} className="text-[#2E5090] shrink-0" />
          <div>
            <p className="font-black text-[15px] text-[#1a2d4a]">Akun terhubung</p>
            <p className="text-[13px] text-[#3d6b5e]">{loggedInUser.fullName ?? loggedInUser.email}</p>
            <p className="text-[12px] text-[#7a9a8f]">{loggedInUser.email}</p>
          </div>
        </div>
        <p className="text-[14px] text-[#3d6b5e]">Lanjutkan mengisi profil tukang Anda.</p>
      </div>
    );
  }
