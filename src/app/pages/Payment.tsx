...
const JOB = {
  ...
  emoji: "🚨",
  tasker: { name: "Andi S.", initials: "AS", color: "#2E5090", rating: 5.0, reviews: 134 },
  priceRaw: 320000,
  serviceFee: 16000,
  insurance: 5000,
};
...
const METHODS: {
  ...
  },
  {
    ...
    color: "#1a2d4a",
    bg: "#F5F1E8",
    logo: <CreditCard size={24} className="text-[#1a2d4a]" />,
...
function VAPendingScreen({ method }: { method: PayMethod }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-[#F5F1E8] border-2 border-[#F59E42] rounded-2xl p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="shrink-0">{m.logo}</div>
          <div>
            <p className="text-[11px] font-bold text-[#7a9a8f] uppercase tracking-wider">Nomor Virtual Account</p>
            <p className="font-black text-[22px] text-[#1a2d4a] tracking-widest">{vaNumber}</p>
          </div>
          <button onClick={copy} className="ml-auto flex items-center gap-1.5 text-[#2E5090] font-bold text-[12px] border border-[#F59E42] px-3 py-1.5 rounded-lg hover:bg-[#f0f7f4] transition-colors">
        ...
        <div className="flex items-center justify-between bg-white rounded-xl px-4 py-3">
          <div>
            <p className="text-[11px] text-[#7a9a8f] font-semibold">Total pembayaran</p>
            <p className="font-black text-[20px] text-[#2E5090]">{formatRp(TOTAL)}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] text-[#7a9a8f] font-semibold">Batas waktu</p>
            <div className="flex items-center gap-1 text-[#1e3d7a]">
              <Clock size={13} />
      ...
      <div>
        <p className="font-bold text-[13px] text-[#0f2035] mb-3">Cara membayar:</p>
        <div className="space-y-2">
          {(steps[method] ?? steps.bca).map((step, i) => (
            <div key={i} className="flex items-start gap-3 text-[13px] text-[#1a3d5c]">
              <div className="w-5 h-5 rounded-full bg-[#f0f7f4] border border-[#F59E42] flex items-center justify-center text-[10px] font-black text-[#2E5090] shrink-0 mt-0.5">
                {i + 1}
              </div>
              {step}
            </div>
      ...
      <p className="text-center text-[12px] text-[#7a9a8f]">
...
function CardForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div>
        <label className="block text-[13px] font-bold text-[#0f2035] mb-1.5">Nomor Kartu</label>
        <div className="relative">
          <input
            value={num}
            onChange={(e) => setNum(formatNum(e.target.value))}
            placeholder="0000 0000 0000 0000"
            className="w-full border-2 border-[#b8d4c8] rounded-xl px-4 py-3 pr-12 text-[15px] font-mono text-[#0f2035] placeholder-[#7a9a8f] bg-[#F5F1E8] outline-none focus:border-[#2E5090] transition-all tracking-widest"
          />
      ...
      <div>
        <label className="block text-[13px] font-bold text-[#0f2035] mb-1.5">Nama Pemegang Kartu</label>
        <input
          value={name}
          onChange={(e) => setName(e.target.value.toUpperCase())}
          placeholder="BUDI SANTOSO"
          className="w-full border-2 border-[#b8d4c8] rounded-xl px-4 py-3 text-[14px] text-[#0f2035] placeholder-[#7a9a8f] bg-[#F5F1E8] outline-none focus:border-[#2E5090] transition-all uppercase tracking-wide"
        />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-[13px] font-bold text-[#0f2035] mb-1.5">Masa Berlaku</label>
          <input
            value={exp}
        ...
        <div>
          <label className="block text-[13px] font-bold text-[#0f2035] mb-1.5 flex items-center gap-1">
            CVV <Lock size={11} className="text-[#7a9a8f]" />
          </label>
          <input
            value={cvv}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-5">
      ...
      <div className="text-center">
        <p className="font-black text-[20px] text-[#1a2d4a]">Memproses pembayaran…</p>
        <p className="text-[13px] text-[#3d6b5e] mt-1">Mohon jangan tutup halaman ini</p>
      </div>
      <div className="flex gap-1.5 mt-2">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="w-2 h-2 rounded-full bg-[#2E5090]"
...
function SuccessScreen() {
  ...
  return (
    <div className="flex flex-col items-center text-center gap-5 py-4">
      ...
      {/* Receipt card */}
      <div className="w-full bg-[#F5F1E8] border border-[#c8dfd8] rounded-2xl overflow-hidden text-left">
        <div className="bg-[#1a2d4a] px-5 py-4 flex items-center justify-between">
          <div>
          ...
          <div className="text-right">
            ...
            <p className="font-black text-[18px] text-[#F59E42]">{formatRp(TOTAL)}</p>
          </div>
        </div>

        <div className="flex items-center">
          <div className="w-4 h-4 rounded-full bg-[#F5F1E8] -ml-2 shrink-0" />
          <div className="flex-1 border-t-2 border-dashed border-[#b8d4c8]" />
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-[#7a9a8f] font-semibold">{JOB.category}</p>
              <p className="font-bold text-[14px] text-[#0f2035] line-clamp-1">{JOB.title}</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5 text-[13px]">
            {[
              ...
              <div key={label} className="flex justify-between text-[#3d6b5e]">
                <span>{label}</span><span>{val}</span>
              </div>
            ))}
            <div className="border-t border-[#b8d4c8] pt-1.5 flex justify-between font-black text-[#1a2d4a]">
              <span>Total</span><span>{formatRp(TOTAL)}</span>
            </div>
          </div>

          {/* Tasker row */}
          <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-[#c8dfd8]">
            ...
            <div className="flex items-center gap-1 text-[#f59e0b]">
              ...
              <span className="font-bold text-[12px] text-[#0f2035]">{JOB.tasker.rating}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#F5F1E8]" style={{ fontFamily: "Manrope, sans-serif" }}>
      ...
      <div className="bg-white border-b border-[#f5eded] sticky top-0 z-10">
        <div className="flex items-center justify-between px-6 py-4 max-w-[860px] mx-auto">
          <Link to="/tasks" className="flex items-center gap-1.5 text-[13px] font-semibold text-[#3d6b5e] hover:text-[#2E5090] transition-colors">
            <ChevronLeft size={16} /> Kembali
          </Link>
          <p className="font-black text-[15px] text-[#1a2d4a]">Pembayaran</p>
        ...
        {(screen === "method" || screen === "confirm") && (
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="font-black text-[22px] text-[#1a2d4a] mb-5">
                {screen === "confirm" && selected === "card" ? "Detail Kartu" : "Pilih Metode Pembayaran"}
              </h1>

              {/* Card form */}
              {screen === "confirm" && selected === "card" ? (
                <div className="bg-white rounded-2xl border border-[#c8dfd8] p-6">
                  <button onClick={() => setScreen("method")} className="flex items-center gap-1 text-[13px] font-semibold text-[#3d6b5e] hover:text-[#2E5090] mb-5 transition-colors">
                    <ChevronLeft size={14} /> Ganti metode
                  </button>
                  <div className="flex items-center gap-3 mb-5 bg-[#F5F1E8] rounded-xl px-4 py-3">
                    <div className="shrink-0">{selectedM?.logo}</div>
                    <div>
                      <p className="font-bold text-[14px] text-[#0f2035]">{selectedM?.label}</p>
                      <p className="text-[12px] text-[#3d6b5e]">{selectedM?.desc}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  {groups.map((group) => (
                    <div key={group} className="bg-white rounded-2xl border border-[#c8dfd8] overflow-hidden">
                      <div className="flex items-center gap-2 px-5 py-3 border-b border-[#c8dfd8] bg-[#F5F1E8]">
                        {group === "E-Wallet" && <Smartphone size={15} className="text-[#2E5090]" />}
                        {group === "Transfer Bank" && <Building2 size={15} className="text-[#2E5090]" />}
                        {group === "Kartu" && <CreditCard size={15} className="text-[#2E5090]" />}
                        <span className="font-bold text-[12px] text-[#1a3d5c] uppercase tracking-wider">{group}</span>
                      </div>
                      <div className="divide-y divide-[#f5eded]">
                        {METHODS.filter((m) => m.group === group).map((m) => (
                          <button
                            ...
                            className={`w-full flex items-center gap-4 px-5 py-4 text-left transition-all hover:bg-[#F5F1E8] ${selected === m.id ? "bg-[#f0f7f4]" : ""}`}
                          >
                            <div className="shrink-0">{m.logo}</div>
                            <div className="flex-1 min-w-0">
                              <p className={`font-bold text-[14px] ${selected === m.id ? "text-[#2E5090]" : "text-[#0f2035]"}`}>
                                {m.label}
                              </p>
                              <p className="text-[12px] text-[#3d6b5e]">{m.desc}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                                selected === m.id ? "border-[#2E5090] bg-[#2E5090]" : "border-[#b8d4c8]"
                              }`}>
            ...
            <div className="w-full lg:w-[340px] shrink-0 sticky top-[73px]">
              <div className="bg-white rounded-2xl border border-[#c8dfd8] overflow-hidden">
                {/* Header */}
                <div className="bg-[#1a2d4a] px-5 py-4">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-[11px] text-[#7a9a8f] font-semibold">{JOB.category}</p>
                      <p className="font-bold text-[13px] text-[#0f2035] leading-snug">{JOB.title}</p>
                    </div>
                  </div>

                  {/* Tasker */}
                  <div className="flex items-center gap-3 bg-[#F5F1E8] rounded-xl px-3 py-2.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[11px] text-[#3d6b5e]">{JOB.tasker.rating} · {JOB.tasker.reviews} ulasan</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={label} className="flex justify-between text-[13px] text-[#3d6b5e]">
                        <span>{label}</span>
                        <span>{formatRp(val)}</span>
                      </div>
                    ))}
                    <div className="border-t border-[#c8dfd8] pt-2 flex justify-between font-black text-[#1a2d4a] text-[16px]">
                      <span>Total</span>
                      <span className="text-[#2E5090]">{formatRp(TOTAL)}</span>
                    </div>
                  </div>

                  {/* Safety note */}
                  <div className="flex items-start gap-2 text-[11px] text-[#3d6b5e] bg-[#f0fdf4] rounded-xl p-3 border border-[#bbf7d0]">
                  ...
                  {screen === "confirm" && selected !== "card" && (
                    <div className="space-y-2">
                      <div className="flex items-center gap-3 bg-[#f0f7f4] border border-[#F59E42] rounded-xl px-4 py-3">
                        <div className="shrink-0">{selectedM?.logo}</div>
                        <div className="flex-1">
                          <p className="font-bold text-[13px] text-[#0f2035]">{selectedM?.label}</p>
                          <p className="text-[11px] text-[#3d6b5e]">{selectedM?.desc}</p>
                        </div>
                        <button onClick={() => setScreen("method")} className="text-[11px] text-[#2E5090] font-bold hover:underline shrink-0">Ganti</button>
                      </div>
                      <button
                        onClick={handlePay}
                        className="w-full flex items-center justify-center gap-2 bg-[#2E5090] hover:bg-[#1e3d7a] text-white font-bold text-[15px] py-3.5 rounded-2xl transition-colors"
                      >
              ...
              <div className="flex flex-wrap justify-center gap-4 mt-4">
                {[
                  ...
                  <div key={b.text} className="flex items-center gap-1.5 text-[11px] text-[#7a9a8f] font-semibold">
