...
function Avatar({ initials, id, size = "sm" }: { initials: string | null | undefined; id: string; size?: "sm" | "lg" }) {
  ...
  if (!initials) {
    return (
      <div className={`${cls} flex items-center justify-center`} style={{ background: "#EEF3FB" }}>
...
function StarRow({ rating, count }: { rating: number; count: number }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[12px] text-[#58708D]">{rating} ({count} ulasan)</span>
    </div>
  );
}
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl border cursor-pointer transition-all px-5 py-4 hover:shadow-sm ${
        selected ? "border-[#1D4196] shadow-md ring-1 ring-[#1D4196]/20" : "border-[#D8E2F0] hover:border-[#FD6665]"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-[14px] leading-snug text-[#172E4D] mb-2">{task.title}</h3>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <span className="font-black text-[15px] text-[#172E4D]">{task.price}</span>
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="shrink-0 border-b border-[#f5eded]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`w-9 h-9 rounded-full border flex items-center justify-center transition-all ${
                saved ? "border-[#1D4196] bg-[#EEF3FB]" : "border-[#D8E2F0] hover:border-[#1D4196]"
              }`}
            >
              <Heart size={16} className={saved ? "text-[#1D4196] fill-[#1D4196]" : "text-[#58708D]"} />
            </button>
        ...
        <div className="flex items-start justify-between gap-3 px-6 pb-3">
          <h2 className="min-w-0 break-words font-black text-[18px] text-[#172E4D] leading-snug">{task.title}</h2>
          <div className="text-right shrink-0">
            <p className="font-black text-[20px] text-[#172E4D] leading-none">{task.price}</p>
          ...
          {visibleTabs.map((t) => (
            ...
            >
              ...
              {t.count !== null && t.count > 0 && (
                <span className={`ml-1 text-[10px] font-black px-1.5 py-0.5 rounded-full ${tab === t.id ? "bg-[#1D4196] text-white" : "bg-[#D8E2F0] text-[#58708D]"}`}>
                  {t.count}
                </span>
              )}
              {tab === t.id && (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {tab === "detail" && (
          <div className="px-6 py-5">
            <div className="flex flex-wrap gap-2 mb-5">
              {task.flexible && (
                <div className="flex items-center gap-1.5 bg-[#F7F9FC] rounded-lg px-3 py-2 text-[13px] text-[#294566] font-medium">
                  <Calendar size={13} className="text-[#1D4196]" /> Waktu fleksibel
                </div>
              )}
              {task.date && !task.flexible && (
                <div className="flex items-center gap-1.5 bg-[#F7F9FC] rounded-lg px-3 py-2 text-[13px] text-[#294566] font-medium">
              ...
              <div className="flex items-center gap-1.5 bg-[#F7F9FC] rounded-lg px-3 py-2 text-[13px] text-[#294566] font-medium">
                <MapPin size={13} className="text-[#1D4196]" /> Di lokasi · Jakarta
              </div>
            </div>

            <h3 className="font-bold text-[12px] text-[#7890AA] uppercase tracking-wider mb-3">Deskripsi pekerjaan</h3>
            ...
            <div className="flex items-start gap-3 bg-[#F7F9FC] rounded-xl p-4">
              ...
              <div>
                <p className="text-[13px] font-bold text-[#172E4D]">Pembayaran terlindungi</p>
                <p className="text-[12px] text-[#58708D] mt-0.5">Dana baru dicairkan setelah pekerjaan selesai dikonfirmasi.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "penawaran" && (
          <div className="px-6 py-5">
            {loadingOffers ? (
              <p className="text-center py-12 text-[#7890AA] text-[14px]">Memuat penawaran…</p>
            ) : offers.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                {offers.map((offer) => {
                  ...
                  return (
                  <div key={offer.id} className="bg-[#F7F9FC] border border-[#D8E2F0] rounded-2xl p-4">
        ...
        {tab === "pemilik" && task.poster && (
          <div className="px-6 py-5">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-[16px] text-[#172E4D]">{task.poster.name}</p>
                <StarRow rating={task.poster.rating} count={task.poster.reviews} />
                <div className="flex flex-wrap gap-3 mt-3">
                  <div className="text-center">
                    <p className="font-black text-[15px] text-[#172E4D]">{task.poster.completionRate}%</p>
                    <p className="text-[11px] text-[#7890AA]">Penyelesaian</p>
                  </div>
                  <div className="w-px bg-[#EEF3FB]" />
                  <div className="text-center">
                    <p className="font-black text-[15px] text-[#172E4D]">{task.poster.reviews}</p>
                    <p className="text-[11px] text-[#7890AA]">Ulasan</p>
                  </div>
                  <div className="w-px bg-[#EEF3FB]" />
                  <div className="text-center">
                    <p className="font-black text-[15px] text-[#172E4D]">Sejak {task.poster.memberSince}</p>
      ...
      <div className="bg-white border-b border-[#f5eded] shrink-0 shadow-sm">
        <div className="flex items-center gap-2 px-6 py-3 max-w-[1400px] mx-auto w-full overflow-x-auto">
          ...
          <button className="flex items-center gap-1.5 text-[13px] font-semibold text-[#294566] bg-white border border-[#D8E2F0] rounded-lg px-4 py-[9px] hover:border-[#1D4196] hover:text-[#1D4196] transition-all whitespace-nowrap shrink-0">
            <MapPin size={13} className="text-[#1D4196]" /> Jakarta & sekitarnya <ChevronDown size={13} />
          </button>
          <button className="flex items-center gap-1.5 text-[13px] font-semibold text-[#294566] bg-white border border-[#D8E2F0] rounded-lg px-4 py-[9px] hover:border-[#1D4196] hover:text-[#1D4196] transition-all whitespace-nowrap shrink-0">
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0">
        ...
        <div className={taskListClassName}>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-2.5">
            {loading ? (
              <p className="text-center py-16 text-[#7890AA] text-[14px]">Memuat pekerjaan…</p>
            ) : filtered.map((task) => (
              <TaskCard
                key={task.id}
                task={task}
