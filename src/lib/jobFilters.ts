...
/** Jakarta metro bounds for pin placement */
const MAP_BOUNDS = { north: -6.08, south: -6.42, west: 106.62, east: 107.05 };

/** Jakarta default center */
export const JAKARTA_CENTER = { lat: -6.2088, lng: 106.8456 };

/** Area centroid with small deterministic jitter (~±400 m) so pins don't stack. Never uses stored exact coords. */
export function getPublicMapCoordinates(job: Job): { lat: number; lng: number } | null {
  const base = AREA_COORDS[job.area];
  if (!base) return null;
...
/** @deprecated Use getPublicMapCoordinates for browse/map UI */
export function getJobCoordinates(job: Job): { lat: number; lng: number } | null {
  return getPublicMapCoordinates(job);
}
